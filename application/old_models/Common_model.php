...
class Common_model extends CI_Model {
    ...
    function upload_image($max_size){
		...
		function get_phonecode($id){
				$this->db->select('id, CONCAT(sortname, CONCAT(" (+", phonecode, ")")) AS phonecode ');
				$this->db->from('country');
				$this->db->where('id', $id);
				$this->db->order_by('id','ASC');
				$query = $this->db->get();
		...
		function get_all_country_by_phonecode($id){
			//echo $phonecode;exit;
				$this->db->select('id, name');
				$this->db->from('country');
				$this->db->where('id', $id);
	 ...
	 public function select_contact_data($email){
	 	 $this->db->select('*');
...
function select_row($table,$user_id){
			 $this->db->select();
			 $this->db->from($table);
		...
		function get_user_exist($username){
		...
		function getUsersName_by_id($id){
			$this->db->select('username');
			$this->db->from('user');
			$this->db->where('id =', $id);
			$query = $this->db->get();
 ...
 //-- get single user info
    function get_single_user_info($username){
        $this->db->select('*');
        $this->db->from('user');
        $this->db->where('username',$username);
        $query = $this->db->get();
